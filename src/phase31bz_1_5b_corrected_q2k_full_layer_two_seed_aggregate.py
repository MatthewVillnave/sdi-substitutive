#!/usr/bin/env python3
"""
phase31bz_1_5b_corrected_q2k_full_layer_two_seed_aggregate.py

Phase 31BZ — Qwen2.5-1.5B Corrected Q2_K Full-Layer Two-Seed Aggregate.

Scope (strictly per 31BY/31BZ spec — standalone tensor-harness aggregate only,
NOT generation, NOT inference, NOT llama.cpp runtime integration):
- 28 layers [0..27] x 2 seeds [0, 9] = 56 anchor pairs total (full layer coverage).
...
- Per-pair metrics: cos_low, cos_sub, delta_cos, MAE_low, MAE_sub, MAE_delta,
  severe flag (delta_cos < -0.05), finite checks, shapes.
- Per-layer memory accounting: q2k_up + sdir_up + q2k_gate + sdir_gate + q2k_down
  vs 3 x Q4_budget_family for the 1.5B shape.
- Per-layer/family tensor metadata (raw_gguf_shape, dequant_shape, tensor_type,
  n_elements) for source-GGUF observation.
- Per-pair wall-clock timing (informational only, recorded in result, NOT used
  for any claim).

Forbidden (per 31BZ spec -- recorded in result, not just claimed):
...
SDI_LLAMA_CPP_BUILD / SDI_LLAMA_CPP_ROOT) -- same convention as 31BJ/31BU/31BV/31BX.

This is a derived runner from 31BX (parameterized to all 28 layers). Same harness,
same policy, same W_ref source.
"""
...
# 31BZ scope: ALL 28 layers x 2 seeds = 56 anchor pairs (full layer coverage)
...
OUT_JSON = OUT_DIR / "PHASE31BZ_1_5B_CORRECTED_Q2K_FULL_LAYER_TWO_SEED_AGGREGATE.json"
...
def get_model_path() -> str:
    ...
    if not sdi:
        raise RuntimeError("BLOCKED_31BZ_SDI_MODEL_DIR_UNSET")
    p = Path(sdi) / MODEL_REL
    if not p.is_file():
        raise RuntimeError(f"BLOCKED_31BZ_MODEL_FILE_MISSING: {p}")
    return str(p)
...
def load_layer_mlp(gguf_path: str, layer: int):
    ...
    try:
        ...
        raise RuntimeError(f"BLOCKED_31BZ_GGUF_READER_FAIL: {e}")
    ...
    for fam, name in targets.items():
        ...
        for cand in reader.tensors:
            if cand.name == name:
                ...
                break
        if t is None:
            raise RuntimeError(f"BLOCKED_31BZ_GGUF_READER_FAIL: tensor {name} not found")
        try:
            w = np.asarray(dequantize(t.data, t.tensor_type))
        except Exception as e:
            raise RuntimeError(f"BLOCKED_31BZ_DEQUANT_FAIL ({name}): {e}")
        out[fam] = {
            "name": t.name,
...
def main():
    ...
    if not q2k_is_available():
        return {
            "classification": "BLOCKED_31BZ_Q2K_BACKEND_FAIL",
    ...
    tmpdir = tempfile.mkdtemp(prefix="phase31bz_")

    # --- Tensor diagnostic (per layer) -----------------------------
    # Loaded BEFORE the per-layer summary so the summary can include
    # real tensor metadata (avoids the 31BV "null tensor metadata" issue).
    ...
    # Run the 56 anchor pairs (full layer coverage, 2 seeds)
    import time  # local import: only used for informational per-pair wall-clock timing
    anchor_results = []
    per_pair_timing_seconds = []  # informational only, NOT used for any claim
    ...
    for layer in LAYERS:
        for seed in SEEDS:
            ...
            pair_seconds = t_pair_end - t_pair_start
            per_pair_timing_seconds.append({
                "layer": layer,
                "seed": seed,
            ...
            print(
                f"L{layer:>2} S{seed}  cos_low={r['cos_low']:+.6f}  "
                f"delta_cos={r['delta_cos']:+.6f}  MAE_delta={r['MAE_delta']:+.6f}  "
                f"severe={r['severe']}  margin={r['memory']['per_layer_margin']:+,}  "
                f"pair_time={pair_seconds:.2f}s"
            )
    t_run_end = time.perf_counter()
    total_wall_clock_seconds = t_run_end - t_run_start

    # --- Aggregate summary (within the 14-pair stratified scope -- not aggregate) ---
    ...
    # --- Per-layer summary (7 layers x 2 seeds each) ---------------
    per_layer_summary = {}
    for layer in LAYERS:
    ...
    # --- Tensor-type observations (source-GGUF) --------------------
    # Per-family per-layer table for the selected layers.
    # No "blanket Q6_K" claim -- one row per layer.
    tensor_type_table = {
        str(layer): {
            "ffn_up": tensor_diag_per_layer[str(layer)]["ffn_up"]["tensor_type"],
            "ffn_gate": tensor_diag_per_layer[str(layer)]["ffn_gate"]["tensor_type"],
            "ffn_down": tensor_diag_per_layer[str(layer)]["ffn_down"]["tensor_type"],
        }
        for layer in LAYERS
        if str(layer) in tensor_diag_per_layer
    }

    # --- Classification (per 31BY criteria exactly) ----------------
    # HARD gates (no allowance):
    #   - regression passes before AND after (enforced at agent layer)
    #   - 56/56 finite
    #   - 56/56 memory-positive
    #   - 0 severe regressions
    #   - no model files / blobs / scope creep (enforced at agent layer)
    # SOFT thresholds (2-pair allowance applies ONLY here):
    #   - >= 54/56 cos+ (independent)
    #   - >= 54/56 MAE+ (independent)
    if n != len(LAYERS) * len(SEEDS):
        classification = "BLOCKED_31BZ_INTERNAL_PAIR_COUNT_MISMATCH"
        reason = f"expected {len(LAYERS) * len(SEEDS)} pairs, got {n}"
    elif n_finite < n:
        # 1+ non-finite pair => not CLEAN. Severity routing:
        #   >= 3 non-finite => BLOCKED (structural harness failure)
        #   1-2 non-finite => PARTIAL_TRADEOFF
        if (n - n_finite) >= 3:
            classification = "BLOCKED_31BZ_NONFINITE_OUTPUTS"
            reason = f"{n - n_finite}/{n} pairs produced non-finite Y_ref/Y_low/Y_sub (>= 3 is structural harness failure)"
        else:
            classification = "PARTIAL_31BZ_1_5B_Q2K_AGGREGATE_TRADEOFF"
            reason = f"{n - n_finite}/{n} pairs non-finite (1-2 non-finite => PARTIAL_TRADEOFF, not BLOCKED)"
    elif n_mem_pos < n:
        # ANY single memory-negative => PARTIAL_MEMORY_FAIL (HARD, no count threshold)
        classification = "PARTIAL_31BZ_1_5B_Q2K_AGGREGATE_MEMORY_FAIL"
        reason = f"only {n_mem_pos}/{n} pairs memory-positive under selected policy (HARD gate; ANY memory-negative => PARTIAL_MEMORY_FAIL)"
    elif n_severe > 0:
        # ANY single severe regression => PARTIAL_TRADEOFF (HARD), unless also memory-negative
        # (memory-negative check above already catches that case first)
        classification = "PARTIAL_31BZ_1_5B_Q2K_AGGREGATE_TRADEOFF"
        reason = f"{n_severe}/{n} pairs have severe regression (delta_cos < {SEVERE_DELTA_COS}); HARD gate"
    elif n_cos_pos < (n - 2) or n_mae_imp < (n - 2):
        # 3+ pairs not cos+ or 3+ pairs not MAE+ => allowances exceeded
        classification = "PARTIAL_31BZ_1_5B_Q2K_AGGREGATE_MINOR_FAILURES"
        reason = f"{n_cos_pos}/{n} cosine-improved, {n_mae_imp}/{n} MAE-improved; >= 3 pairs failing either threshold => allowances (2-pair each, independent) exceeded"
    else:
        # All HARD gates pass; soft thresholds pass within 2-pair allowance each (independent)
        classification = "PASS_31BZ_1_5B_Q2K_FULL_LAYER_TWO_SEED_AGGREGATE_CLEAN"
        reason = f"all {n}/{n} pairs memory-positive, cosine-improved, MAE-improved, 0 severe, all finite; soft thresholds within 2-pair allowance (cos+: {n_cos_pos}/{n}, MAE+: {n_mae_imp}/{n})"

    # --- Assemble result ------------------------------------------
    result = {
        "phase": "31BZ",
        "title": "Phase 31BZ -- Qwen2.5-1.5B Corrected Q2_K Full-Layer Two-Seed Aggregate",
        "scope": (
            "standalone tensor-harness aggregate only -- "
            f"layers {LAYERS} x seeds {SEEDS} = {len(LAYERS) * len(SEEDS)} anchor pairs, "
            "corrected_ceil_per_row Q2_K + ffn_up/ffn_gate SDIR k=0.5% + ffn_down W_low only. "
            "NO generation/inference, NO llama.cpp runtime integration, NO FP16 W_ref."
        ),
        ...
        "config": {
            ...
            "layers": LAYERS,
            "seeds": SEEDS,
            "route": "31BY Option A: full 28-layer x 2-seed standalone tensor-harness aggregate = 56 anchor pairs",
            "n_layers_in_1_5B": 28,
            "coverage_pct": round(100.0 * len(LAYERS) / 28.0, 2),
        },
        "tensors_read_per_layer": tensor_diag_per_layer,
        "tensor_type_table": tensor_type_table,
        "memory_accounting": {
        ...
        "summary": {
            ...
            "max_per_layer_margin": max(r["memory"]["per_layer_margin"] for r in anchor_results),
            "n_layers": len(LAYERS),
            "n_seeds": len(SEEDS),
        },
        "wall_clock_runtime_seconds_total": total_wall_clock_seconds,
        "wall_clock_runtime_seconds_per_pair": per_pair_timing_seconds,
        "wall_clock_runtime_note": "INFORMATIONAL ONLY. Not used for any claim. Captured as time.perf_counter() around each pair's run_anchor call. Excludes model open/tensor-load cost on the first layer.",
        ...
        "next_allowed_phase_if_partial_or_blocked": (
            "Phase 31BZ-R -- Aggregate Result Forensics / Repair, "
            "only if explicitly requested"
        ),
        "interpretation": (
            "Full-layer two-seed standalone tensor-harness aggregate under selected "
            "corrected_q2k_policy_v1 on Qwen2.5-1.5B. W_ref is the 1.5B Q4_K_M dequantized "
            "tensor (NOT FP16). PASS requires ALL hard gates (regression passes; 56/56 "
            "finite; 56/56 memory-positive; 0 severe; no model files / blobs / scope "
            "creep) AND soft thresholds within 2-pair allowance (cos+ >= 54/56; MAE+ >= "
            "54/56; the two allowances are independent). PARTIAL_MINOR_FAILURES if 3+ "
            "pairs fail cos+ or 3+ pairs fail MAE+; PARTIAL_TRADEOFF if 1-2 non-finite "
            "or any severe; PARTIAL_MEMORY_FAIL if ANY single memory-negative; BLOCKED "
            "on env/model/Q2K/SDIR/regression failure or >= 3 non-finite. This is an "
            "aggregate (28 of 28 layers = 100% layer coverage) but is NOT generation, "
            "NOT inference, NOT llama.cpp runtime integration, NOT a larger-model claim, "
            "NOT a 0.5B comparison, NOT a broader-family claim."
        ),
    }
