#!/usr/bin/env python3
"""
phase31bx_1_5b_corrected_q2k_stratified_layer_probe.py

Phase 31BX — Qwen2.5-1.5B Corrected Q2_K Stratified Layer Probe.

Scope (strictly per 31BX spec — small stratified standalone tensor-harness
probe only, NOT aggregate validation):
- 7 layers [0, 4, 8, 12, 16, 20, 27] x 2 seeds [0, 9] = 14 anchor pairs total.
...
Forbidden (per 31BX spec -- recorded in result, not just claimed):
...
SDI_LLAMA_CPP_BUILD / SDI_LLAMA_CPP_ROOT) -- same convention as 31BJ/31BU/31BV.
"""
...
# 31BW-selected stratified scope: 7 layers x 2 seeds = 14 anchor pairs
...
OUT_JSON = OUT_DIR / "PHASE31BX_1_5B_CORRECTED_Q2K_STRATIFIED_LAYER_PROBE.json"
...
def get_model_path() -> str:
    ...
    if not sdi:
        raise RuntimeError("BLOCKED_31BX_SDI_MODEL_DIR_UNSET")
    p = Path(sdi) / MODEL_REL
    if not p.is_file():
        raise RuntimeError(f"BLOCKED_31BX_MODEL_FILE_MISSING: {p}")
    return str(p)
...
def load_layer_mlp(gguf_path: str, layer: int):
    ...
    try:
        ...
        raise RuntimeError(f"BLOCKED_31BX_GGUF_READER_FAIL: {e}")
    ...
    for fam, name in targets.items():
        ...
        for cand in reader.tensors:
            if cand.name == name:
                ...
                break
        if t is None:
            raise RuntimeError(f"BLOCKED_31BX_GGUF_READER_FAIL: tensor {name} not found")
        try:
            w = np.asarray(dequantize(t.data, t.tensor_type))
        except Exception as e:
            raise RuntimeError(f"BLOCKED_31BX_DEQUANT_FAIL ({name}): {e}")
        out[fam] = {
            "name": t.name,
...
def main():
    ...
    if not q2k_is_available():
        return {
            "classification": "BLOCKED_31BX_Q2K_BACKEND_FAIL",
    ...
    tmpdir = tempfile.mkdtemp(prefix="phase31bx_")
    ...
    if n != len(LAYERS) * len(SEEDS):
        classification = "BLOCKED_31BX_INTERNAL_PAIR_COUNT_MISMATCH"
        reason = f"expected {len(LAYERS) * len(SEEDS)} pairs, got {n}"
    elif n_finite < n:
        classification = "BLOCKED_31BX_NONFINITE_OUTPUTS"
        reason = f"{n - n_finite}/{n} pairs produced non-finite Y_ref/Y_low/Y_sub"
    elif n_mem_pos < n:
        classification = "PARTIAL_31BX_1_5B_Q2K_STRATIFIED_LAYER_MEMORY_FAIL"
        reason = f"only {n_mem_pos}/{n} pairs memory-positive under selected policy"
    elif n_severe > 0:
        classification = "PARTIAL_31BX_1_5B_Q2K_STRATIFIED_LAYER_TRADEOFF"
        ...
        classification = "PARTIAL_31BX_1_5B_Q2K_STRATIFIED_LAYER_MINOR_FAILURES"
    ...
    result = {
        "phase": "31BX",
        "title": "Phase 31BX -- Qwen2.5-1.5B Corrected Q2_K Stratified Layer Probe",
        "scope": (
            "small stratified standalone tensor-harness probe only -- "
        ...
        "config": {
            ...
            "layers": LAYERS,
            "seeds": SEEDS,
            "route": "31BW Option A: 7-layer stratified x 2 seeds = 14 anchor pairs",
            "n_layers_in_1_5B": 28,
        ...
        "next_allowed_phase_if_blocked": (
            "Phase 31BX-R -- Stratified Layer Probe Repair, only if explicitly requested"
        ),
        "interpretation": (
            "Stratified standalone tensor-harness probe under selected corrected_q2k_policy_v1 "
            "on Qwen2.5-1.5B. W_ref is the 1.5B Q4_K_M dequantized tensor (NOT FP16). PASS "
            "requires all 14 pairs: memory-positive, cosine-improved, MAE-improved, no severe "
            "regressions, all finite. PARTIAL on any minor failure; severe regression OR memory "
            "fail triggers the corresponding PARTIAL or BLOCKED classification. This is a "
            "stratified probe (7 of 28 layers = 25% layer coverage), NOT aggregate validation, "
            "NOT a larger-model claim, NOT a 0.5B comparison."
        ),
    }
