#!/usr/bin/env python3
"""
phase31bu_1_5b_corrected_q2k_anchor_probe.py

Phase 31BU — Qwen2.5-1.5B Corrected Q2_K Anchor Probe.

Scope (strictly per 31BU spec — anchor probe only, NO aggregate validation):
...
- Use a tiny deterministic random input (np.random.default_rng(seed), batch 1)
  for the selected seeds (Route A: 0, 9, 13 → 3 anchor pairs).
- Use the Q4_K_M dequantized tensor as W_ref (not FP16 reference).
...
- Compute per-pair metrics: cos_low, cos_sub, delta_cos, MAE_low, MAE_sub,
  MAE_delta, severe flag (delta_cos < -0.05).
- Compute per-layer memory: q2k_up + sdir_up + q2k_gate + sdir_gate + q2k_down
  vs 3 × Q4_budget_family for the 1.5B shape.

Forbidden (per 31BU spec — recorded in result, not just claimed):
- No aggregate validation
- No multi-layer sweep beyond layer 0
- No generation / inference
- No llama.cpp runtime integration
- No performance claim
- No model quality claim
- No model files committed
- No Q2_K / SDIR blobs committed (all temp work under tempfile.mkdtemp)
...
SDI_LLAMA_CPP_BUILD / SDI_LLAMA_CPP_ROOT) — same convention as 31BJ/31BH-R2.
"""
...
# ─── Configuration ────────────────────────────────────────────────────────────
# Model (env var only — no hardcoded private path)
...
# Route A: layer 0 only, 3 anchor pairs (per spec)
LAYERS = [0]
SEEDS = [0, 9, 13]
...
# ffn_up/ffn_gate: (8960, 1536) → 8960 * 1536 / 2 = 6,881,280 bytes
# ffn_down:        (1536, 8960) → 1536 * 8960 / 2 = 6,881,280 bytes (same)
# So Q4_BUDGET_FAMILY is shape-independent in value but per-family.
...
OUT_JSON = OUT_DIR / "PHASE31BU_1_5B_CORRECTED_Q2K_ANCHOR_PROBE.json"
...
def get_model_path() -> str:
    ...
    if not sdi:
        raise RuntimeError("BLOCKED_31BU_SDI_MODEL_DIR_UNSET")
    p = Path(sdi) / MODEL_REL
    if not p.is_file():
        raise RuntimeError(f"BLOCKED_31BU_MODEL_FILE_MISSING: {p}")
    return str(p)
...
def load_layer_mlp(gguf_path: str, layer: int):
    ...
    try:
        ...
        raise RuntimeError(f"BLOCKED_31BU_GGUF_READER_FAIL: {e}")
    ...
    for fam, name in targets.items():
        ...
        for cand in reader.tensors:
            if cand.name == name:
                ...
                break
        if t is None:
            raise RuntimeError(f"BLOCKED_31BU_GGUF_READER_FAIL: tensor {name} not found")
        try:
            w = np.asarray(dequantize(t.data, t.tensor_type))
        except Exception as e:
            raise RuntimeError(f"BLOCKED_31BU_DEQUANT_FAIL ({name}): {e}")
        out[fam] = {
            "name": t.name,
...
def run_anchor(layer: int, seed: int, mode: str, gguf_path: str):
    ...
    """
    # Deterministic X vector — same convention as 31AY/31BA/31BJ/31BK
...
def main():
    ...
    if not q2k_is_available():
        return {
            "classification": "BLOCKED_31BU_Q2K_BACKEND_FAIL",
            "error": "libggml-base.so (with quantize_row_q2_K_ref / dequantize_row_q2_K) not loadable",
        }

    # Sanity: env disclosure (no private path in committed result)
    q2k_lib_path = None
    try:
        q2k_lib_path = q2k_lib.__class__.__module__  # not useful, but try
    except Exception:
        pass
    try:
    ...
    tmpdir = tempfile.mkdtemp(prefix="phase31bu_")

    # Run the 3 anchor pairs
    anchor_results = []
    for layer in LAYERS:
        for seed in SEEDS:
            try:
    ...
    # ── Aggregate summary (within the tiny anchor scope only — not a full sweep) ─
    ...
    # ── Tensor diagnostic (first layer only) ───────────────────────────────
    tensors_diag = load_layer_mlp(gguf_path, LAYERS[0])
    tensor_diag_table = {
        fam: {
            "name": v["name"],
        ...
        for fam, v in tensors_diag.items()
    }

    # ── Classification ──────────────────────────────────────────────────────
    if n != len(LAYERS) * len(SEEDS):
        classification = "BLOCKED_31BU_INTERNAL_PAIR_COUNT_MISMATCH"
        reason = f"expected {len(LAYERS) * len(SEEDS)} pairs, got {n}"
    elif n_finite < n:
        classification = "BLOCKED_31BU_NONFINITE_OUTPUTS"
        reason = f"{n - n_finite}/{n} pairs produced non-finite Y_ref/Y_low/Y_sub"
    elif n_mem_pos < n:
        classification = "PARTIAL_31BU_1_5B_Q2K_ANCHOR_MEMORY_FAIL"
        reason = f"only {n_mem_pos}/{n} pairs memory-positive under selected policy"
    elif n_severe > 0:
        classification = "PARTIAL_31BU_1_5B_Q2K_ANCHOR_TRADEOFF"
        ...
        classification = "PARTIAL_31BU_1_5B_Q2K_ANCHOR_MINOR_FAILURES"
    ...
    result = {
        "phase": "31BU",
        "title": "Phase 31BU — Qwen2.5-1.5B Corrected Q2_K Anchor Probe",
        "scope": (
            "anchor probe only — layer 0, 3 seeds (0, 9, 13) per Route A, "
            "corrected_ceil_per_row Q2_K + ffn_up/ffn_gate SDIR k=0.5% + ffn_down W_low only. "
            "NO aggregate validation, NO multi-layer sweep, NO generation/inference."
        ),
        ...
        "config": {
            ...
            "layers": LAYERS,
            "seeds": SEEDS,
            "route": "A (layer 0 only, 3 anchor pairs)",
        },
        "tensors_read_layer0": tensor_diag_table,
        "memory_accounting": {
            ...
            "Q2K_corrected_ceil_per_row_1_5B_per_family": q2k_lib_path and "see anchor_results[*].memory.q2k_bytes_*",
        },
        ...
        "next_allowed_phase_if_blocked": (
            "Phase 31BU-R — Anchor Probe Repair, only if explicitly requested"
        ),
        "interpretation": (
            "Anchor probe under selected corrected_q2k_policy_v1 on Qwen2.5-1.5B layer 0. "
            "W_ref is the 1.5B Q4_K_M dequantized tensor (NOT FP16). All results phrased "
            "as 'relative to Q4_K_M W_ref'. PASS requires all anchor pairs: memory-positive, "
            "cosine-improved, MAE-improved, no severe regressions, all finite. PARTIAL on "
            "any minor failure; severe regression OR memory fail triggers the corresponding "
            "PARTIAL or BLOCKED classification. This is an anchor probe, NOT aggregate "
            "validation, NOT a larger-model claim."
        ),
    }
...
if __name__ == "__main__":
    ...
    print("\n" + json.dumps({k: v for k, v in out.items() if k in (
        "classification", "classification_reason", "summary", "config"
