...
- If `README.md` and `SOURCE_OF_TRUTH.md` disagree, **`SOURCE_OF_TRUTH.md` wins**.
- This README was last updated in the **README-01** documentation maintenance interlude. The current scientific next phase (per `SOURCE_OF_TRUTH.md` Section 9) is **Phase 31CD — Qwen2.5-1.5B Real-Activation Capture Micro-Probe, only if explicitly requested** (31CC selected 31CD as the next executable phase; default mechanism is an HF-derived real-activation proxy with replay through the existing Q4_K_M GGUF W_ref, conditional on explicit operator approval of the HF safetensors download; fallback is a llama.cpp/GGUF activation-capture instrumentation path, conditional on explicit operator approval of the llama.cpp modification). README-01 did not advance, replace, or invalidate the current scientific next phase. 31CB (Stale File Provenance Cleanup) was a working-tree hygiene + provenance phase, not a new scientific phase. 31CC (Real-Activation Capture Planning) was a planning-only phase, not a new scientific phase. 31CA (Qwen2.5-1.5B Corrected Q2_K Aggregate Freeze / Package Update) was a documentation + provenance + handoff phase, not a new scientific phase.
...
- **Current scientific next phase:** **Phase 31CD — Qwen2.5-1.5B Real-Activation Capture Micro-Probe, only if explicitly requested.** (31CC selected 31CD as the next executable phase. 31CD has not yet begun — Matt must explicitly choose between Option A (HF-derived real-activation proxy, replay through the existing Q4_K_M GGUF W_ref, requires HF safetensors download approval), Option B (llama.cpp/GGUF activation-capture instrumentation, the only path that can claim exact Q4_K_M GGUF runtime activation capture, requires llama.cpp modification approval), or C (stop and re-plan via 31CC-R) at 31CD entry.)
