...
**Phase 31CA — Qwen2.5-1.5B Corrected Q2_K Aggregate Freeze / Package Update, only if explicitly requested.**

Rationale:
- Phase 31BZ PASSED with classification `PASS_31BZ_1_5B_Q2K_FULL_LAYER_TWO_SEED_AGGREGATE_CLEAN`.
- 31BZ ran the 31BY Option A scope: **28 layers [0..27] × seeds [0, 9] = 56 anchor pairs** (28 of 28 layers = 100% layer coverage) on Qwen2.5-1.5B-Instruct Q4_K_M with `corrected_q2k_policy_v1` (corrected_ceil_per_row Q2_K, ffn_up + ffn_gate SDIR k=0.5%, alpha=1.0, no ffn_down residual).
- **56/56 pairs memory-positive, cosine-improved, MAE-improved, 0 severe regressions, all finite.** mean_dc=+0.004758, mean_MAE_imp=−0.012865, median_dc=+0.004501, min per-layer margin +3,380,312 bytes, max +3,380,376 bytes (variance 64 bytes across 28 layers). All 28 layers PASS independently when bucketed.
- Worst pair L26-S9 (Δcos=+0.000390, MAE Δ=−0.029222) — still strongly positive, no severe regression, no memory-negative.
- **Cross-runner / cross-phase reproducibility verified:** L0 result bit-identical to 31BU / 31BV / 31BX (3 prior runners); L4 bit-identical to 31BX; L14 / L27 bit-identical to 31BV. The corrected Q2_K + SDIR + corrected_ceil_per_row pipeline is reproducible across 4 independent runners.
- **Wall clock: 481.5 sec (8.0 min) total for 56 pairs (~8.6 sec/pair).** Informational only — actual was ~3× faster than the 31BY planning estimate of 25 min working / 35 min upper bound.
- 31BZ did NOT run generation, inference, or llama.cpp runtime integration. 31BZ did NOT run a 0.5B comparison. 31BZ did NOT generate any Q2_K / SDIR blobs that were committed (all temp under tempfile.mkdtemp). Prior 0.5B accepted numeric results (31AY / 31BA / 31BM / 31BN / 31BO) and 31BT orientation parity are unchanged. Prior 1.5B accepted facts (31BU / 31BV / 31BW / 31BX / 31BY) are unchanged.
- 31CA scope: aggregate freeze / package update — preserve the 31BZ aggregate as a frozen accepted fact, update the 1.5B-corrected-Q2_K policy documentation, decide whether to extend to a new architecture lane (e.g. 3B / 7B / 14B / different family) or to a runtime-integration lane (explicit approval required for any of these). 31CA does NOT run validation by default; 31CA is a documentation + provenance + handoff phase.
- Stop conditions: orientation regression (re-verify 31BT result), regression failure, model file tracking, scope creep, any aggregate (beyond 56-pair scope) / generation / runtime / quality / behavior / production / speedup / latency claim triggered without explicit approval, any 0.5B-vs-1.5B / 1.5B-vs-3B / 1.5B-vs-7B / cross-family claim triggered without explicit approval, any larger-model validation triggered without explicit approval, or any 1.5B result generalized beyond the 31BU/31BV/31BW/31BX/31BY/31BZ scope.
- The 31BZ artifacts (`docs/PHASE31BZ_1_5B_CORRECTED_Q2K_FULL_LAYER_TWO_SEED_AGGREGATE.md`, `src/phase31bz_1_5b_corrected_q2k_full_layer_two_seed_aggregate.py`, `src/results/PHASE31BZ_1_5B_CORRECTED_Q2K_FULL_LAYER_TWO_SEED_AGGREGATE.json`) and the SOT edit are untracked. They will be committed only if Matt explicitly approves.
