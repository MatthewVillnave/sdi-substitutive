...
Rationale:
- Phase 31CA PASSED with classification `PASS_31CA_1_5B_AGGREGATE_FREEZE_PACKAGE_READY`.
- 31CA is a **freeze + package + provenance + documentation phase only**. NO new validation, NO generation, NO inference, NO llama.cpp runtime integration, NO new scientific work.
- 31CA preserved the 31BZ accepted fact (56/56 pairs PASS on Qwen2.5-1.5B full-layer × 2-seed aggregate) as a frozen checkpoint at commit `f7f2a91d1b904f8f156d6c89584ec0d32229c23e`.
- 31CA updated `corrected_q2k_policy_v1` package (`docs/CORRECTED_Q2K_POLICY_PACKAGE.md` + `src/results/CORRECTED_Q2K_POLICY_PACKAGE.json`) to add 1.5B (31BZ/31CA) as a second evidence tier alongside 0.5B (31BN/31BM). **Policy parameters UNCHANGED. Policy version NOT bumped.** The 1.5B evidence tier is documentation, not code.
- 31CA patched the README conservatively (in-place edits to lines 22, 29, 30, 50) to reflect 31BZ PASS and the new next phase. README structure and tone preserved.
- 31CA proposed a tag (`phase31ca-1_5b-corrected-q2k-aggregate-checkpoint`) at the 31BZ commit but did **NOT** create the tag. Operator must approve tag creation explicitly.
- **31CA did NOT run validation, generation, inference, or llama.cpp runtime integration.** 31CA is a documentation + provenance + handoff phase.
- 31CB scope (recommended default): **stale file provenance cleanup** — the repo still has 5 stale untracked 31BH-R2 / 31BJ files carried through 31BU → 31BZ without resolution. With 1.5B aggregate now frozen, these are the only outstanding provenance debt in the working tree. Cleanup is the lowest-risk next step and removes noise before a new scientific lane is opened.
- Alternative 31CB options (operator may choose instead): 31CB-A Real-Activation Capture Planning (prerequisite for any real-activation claim), 31CB-B Runtime Artifact Format / Loader Planning (prerequisite for any runtime-integration claim). Both are listed in `docs/PHASE31CA_1_5B_AGGREGATE_FREEZE_PACKAGE.md` §17 with rationale.
- Prior accepted numeric results (0.5B 31AY / 31BA / 31BM / 31BN / 31BO; 1.5B 31BS / 31BT / 31BU / 31BV / 31BW / 31BX / 31BY / 31BZ) are unchanged. The 31CA freeze is documentation and package manifest; it does not modify any prior accepted fact.
- Stop conditions: orientation regression (re-verify 31BT result), regression failure, model file tracking, scope creep, any aggregate (beyond the 56-pair 31BZ scope) / generation / runtime / quality / behavior / production / speedup / latency / larger-model claim triggered without explicit approval, any 0.5B-vs-1.5B / 1.5B-vs-3B / 1.5B-vs-7B / cross-family claim triggered without explicit approval, any larger-model validation triggered without explicit approval, or any 1.5B result generalized beyond the 31BU/31BV/31BW/31BX/31BY/31BZ scope.
- The 31CA artifacts (`docs/PHASE31CA_1_5B_AGGREGATE_FREEZE_PACKAGE.md`, `src/results/PHASE31CA_1_5B_AGGREGATE_FREEZE_PACKAGE.json`) and the policy package patches (`docs/CORRECTED_Q2K_POLICY_PACKAGE.md`, `src/results/CORRECTED_Q2K_POLICY_PACKAGE.json`) and the README patch and the SOT edit are untracked. They will be committed only if Matt explicitly approves.
