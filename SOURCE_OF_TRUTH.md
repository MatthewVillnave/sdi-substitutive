...
**Phase 31BZ — Qwen2.5-1.5B Corrected Q2_K Full-Layer Two-Seed Aggregate, only if explicitly requested.**

Rationale:
- Phase 31BY PASSED with classification `PASS_31BY_AGGREGATE_STOP_GO_PLAN_SELECTED`.
- 31BY reviewed 31BU (L0, 3 seeds) + 31BV (L0/L14/L27, 2 seeds) + 31BX (L0/L4/L8/L12/L16/L20/L27, 2 seeds) = 23 cumulative pairs across 3 independent runners, all clean (0 severe, 0 memory-fail, 0 finite-fail). L0 bit-identical across 31BU, 31BV, 31BX. 31BX worst pair (L16-S9) still at Δcos=+0.000652.
- 31BY compared 4 stop/go options and selected **Option A — GO full 28-layer × 2-seed aggregate**: layers 0–27, seeds {0, 9}, 56 pairs. Estimated runtime ~25 min working / ~35 min upper bound based on 31BX 9-min wall-clock observation scaled sub-linearly for single-process 4× pair count.
- 31BY did NOT run validation. 31BY did NOT run aggregate, full 28-layer sweep, generation, inference, or llama.cpp runtime integration. 31BY did NOT run a 0.5B comparison. Prior 0.5B accepted numeric results (31AY / 31BA / 31BM / 31BN / 31BO) and 31BT orientation parity are unchanged.
- 31BZ scope: standalone tensor-harness aggregate only — layers 0–27 × seeds {0, 9} = 56 pairs, `corrected_q2k_policy_v1` (corrected_ceil_per_row Q2_K, ffn_up + ffn_gate SDIR k=0.5%, alpha=1.0, no ffn_down residual), W_ref = 1.5B Q4_K_M dequantized. No generation / inference / runtime integration / speedup / quality / behavior / production claim. No model files committed. No Q2_K/SDIR blobs committed. No FP16 W_ref. No ffn_down residual.
- Success criteria for 31BZ (unambiguous CLEAN definition): regression passes before AND after (PASS_SOURCE_OF_TRUTH_RUNTIME_CLEAN, error_count=0, fallback_count=0, readme_drift_guard.passed=true); all 56 pairs finite (n_finite == 56); all 56 pairs memory-positive (n_memory_positive == 56); 0 severe regressions (n_severe_regressions == 0); >= 54/56 cosine-improved; >= 54/56 MAE-improved; no model files committed; no blobs committed; no scope creep. **The 2-pair allowance applies ONLY to the cos+ and MAE+ soft thresholds (independent). The hard criteria (finite, memory-positive, non-severe, no files/blobs, no scope creep) carry NO allowance — 1 violation of any hard criterion disqualifies CLEAN regardless of cos+/MAE+ counts.**
- Stop conditions: orientation regression (re-verify 31BT result), regression failure, model file tracking, scope creep, any aggregate (beyond 56-pair scope) / generation / runtime / quality / behavior claim triggered without explicit approval, any 0.5B-vs-1.5B comparison triggered without explicit approval, any full 28-layer sweep triggered beyond the 31BZ scope, or any 1.5B result generalized beyond the 31BU/31BV/31BW/31BX/31BY/31BZ scope.
- The 31BY artifacts (`docs/PHASE31BY_AGGREGATE_PLANNING_STOP_GO.md`, `src/results/PHASE31BY_AGGREGATE_PLANNING_STOP_GO.json`) and the SOT edit are untracked. They will be committed only if Matt explicitly approves.
