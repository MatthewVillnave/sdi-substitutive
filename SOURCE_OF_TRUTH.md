...
- **Last verified commit:** `01c20b1058cf69dce05b08366a9bf1f2711ec8f5` (Phase 31CD: run 1.5B real-activation proxy micro-probe, classification `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN`). Previous committed state was `c93bcdf96c8422f91f01fbc8c4e522710922fe61` (Phase 31CB: clean stale phase provenance artifacts); 31CC commit was `dbd10c722264cca0cdaec3a63b1532a324dad034` (intermediate between 31CB and 31CD).
...
- **Current allowed scientific next phase:** **Phase 31CE — Qwen2.5-1.5B Real-Activation Capture Multi-Prompt / Multi-Layer Extension (Option A), only if explicitly requested.** Phase 31CD Option A passed with `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN` (committed at `01c20b10` and pushed to `origin/master`). 31CD was the L0-only, single-prompt, last-prefill-token micro-probe on an HF-derived real-activation proxy (Qwen2.5-1.5B-Instruct safetensors forward pass; replay W_ref = local 1.5B Q4_K_M GGUF dequantized, NO W_ref source pivot; corrected_ceil_per_row Q2_K W_low; ffn_up+ffn_gate SDIR k=0.5%, alpha=1.0, no ffn_down residual; prompt "The capital of France is", 5 BPE tokens, last prefill token id 374, shape [1, 1536], bf16→f32, sha256 of raw X recorded in tempfile and discarded; result JSON records only sha256 + shape + dtype + summary statistics). Result: `cos_low=0.942053`, `cos_sub=0.943183`, `delta_cos=+0.001130`; `MAE_low=0.027586`, `MAE_sub=0.027357`, `MAE_delta=−0.000229`; per-layer margin `+3,380,374` bytes (memory-positive); all finite; 0 severe; classification `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN`. **31CE is the natural next step** (Option A extension to more prompts / more layers / more token positions, only if Matt explicitly requests and the 31CE scope is approved at entry) — **Option A is not** the path to "exact Q4_K_M GGUF runtime activation capture". The alternative next phase is **Phase 31CF — Qwen2.5-1.5B Real-Activation Capture via llama.cpp Instrumentation (Option B), only if explicitly requested and operator permission is granted for llama.cpp modification and rebuild**; Option B is the only path that can claim exact Q4_K_M GGUF runtime activation behavior. Both 31CE and 31CF require explicit operator approval at entry. Phase 31CD-R (Real-Activation Capture Repair) is the recovery branch if 31CD is rejected post-merge.
...
- Phase 31AS (all available layers full MLP Q2_K + low-k residual sweep) — classification `PARTIAL_LAYER_VARIANCE` (corrected from initial `PASS_ALL_LAYERS_MLP_Q2K_LOWK_POLICY_FOUND`):
  ...
  - **31AU PASS:** Classification `PASS_ALL_LAYERS_CONSISTENT_SEED_POLICY_FOUND`:
    ...
    - **Phase 31CD — Qwen2.5-1.5B Real-Activation Capture Micro-Probe (Option A):** Classification `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN` (uncommitted; PRE-COMMIT REPORT pending Matt approval).
...
**Phase 31CE — Qwen2.5-1.5B Real-Activation Capture Multi-Prompt / Multi-Layer Extension (Option A), only if explicitly requested.** Phase 31CD Option A passed (uncommitted; PRE-COMMIT REPORT pending Matt approval) with `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN`. 31CE is the natural next step: extend Option A from the L0-only, single-prompt, last-prefill-token micro-probe (31CD) to more prompts / more layers / more token positions, with explicit operator approval of the 31CE scope at entry. **Option A is NOT** the path to "exact Q4_K_M GGUF runtime activation capture" — that is **Option B** (Phase 31CF), which is the only path that can claim exact Q4_K_M GGUF runtime activation behavior and requires explicit operator permission to modify and rebuild `~/llama.cpp` on this host. Alternative next phase: **Phase 31CF — Qwen2.5-1.5B Real-Activation Capture via llama.cpp Instrumentation (Option B), only if explicitly requested and operator permission is granted for llama.cpp modification and rebuild**. Both 31CE and 31CF require explicit operator approval at entry; the agent does NOT proceed to either without a new request. Recovery branch: **Phase 31CD-R (Real-Activation Capture Repair), only if explicitly requested**.
