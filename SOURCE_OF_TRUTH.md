...
- **SOT version / state label:** v2 (post-31BT, SOT-01 hardened, post-31CD Option A real-activation micro-probe PASSED, committed and pushed)
- **Last verified commit:** `32f030a0d2f9f4a9f208bb6fd653540e3e61ad5c` (Phase 31CD post-push: update SOT to reflect committed/pushed state — the SOT-sync follow-up to the 31CD main commit `01c20b1058cf69dce05b08366a9bf1f2711ec8f5`). 31CD main commit: `01c20b1058cf69dce05b08366a9bf1f2711ec8f5` (Phase 31CD: run 1.5B real-activation proxy micro-probe, classification `PASS_31CD_REAL_ACTIVATION_MICRO_REPLAY_CLEAN`). 31CC commit: `dbd10c722264cca0cdaec3a63b1532a324dad034` (intermediate). 31CB commit: `c93bcdf96c8422f91f01fbc8c4e522710922fe61` (clean stale phase provenance artifacts). 31CE result prepared for commit in this phase; once committed, this entry records the accepted 31CE result.
...
- **Current allowed scientific next phase:** **Phase 31CF — Qwen2.5-1.5B Real-Activation Capture via llama.cpp Instrumentation (Option B), only if explicitly requested and operator permission is granted for llama.cpp modification and rebuild.** Phase 31CE Option A passed with `PASS_31CE_REAL_ACTIVATION_OPTION_A_MULTIPROMPT_MULTILAYER_CLEAN`. 31CE extended 31CD from a 1-pair L0-only single-prompt micro-probe to a 9-pair (3 prompt × 3 layer × last-prefill-token) micro-probe on the same HF-derived real-activation proxy (Qwen2.5-1.5B-Instruct safetensors forward pass; replay W_ref = local 1.5B Q4_K_M GGUF dequantized, NO W_ref source pivot; corrected_ceil_per_row Q2_K W_low; ffn_up+ffn_gate SDIR k=0.5%, alpha=1.0, no ffn_down residual). 31CE prompts: P0 = "The capital of France is" (5 BPE tokens, last token id 374), P1 = "Once upon a time" (4 BPE tokens, last token id 882), P2 = "In a small village" (4 BPE tokens, last token id 14126). Layers: L0, L14, L27. Shape [1, 1536], dtype bf16→f32, sha256 of raw X per pair recorded in tempfile and discarded; result JSON records only sha256 + shape + dtype + summary statistics per pair. Result: 9/9 pairs memory-positive, 9/9 cos non-negative, 9/9 MAE non-worsening, 0/9 severe, all finite; mean_delta_cos=+0.002156, median_delta_cos=+0.001130, mean_MAE_delta=−0.003088; per-layer margin +3,380,350 to +3,380,376 bytes (matches 31BU/31BV/31BX/31BZ/31CD within tolerance). P0 L0 exactly reproduces 31CD (cross-runner reproducibility confirmed). Classification: `PASS_31CE_REAL_ACTIVATION_OPTION_A_MULTIPROMPT_MULTILAYER_CLEAN`. **31CF is the natural next step if the goal is exact Q4_K_M GGUF runtime activation capture** (Option B is the only path that can claim that; requires operator permission to modify and rebuild `~/llama.cpp` on this host). Alternative next phases: **31CE-R (Real-Activation Option A Repair / Prompt-Layer Diagnosis), only if explicitly requested** (recovery branch); **31CG (Option A Larger Prompt/Token Sensitivity Planning), only if explicitly requested** (planning-only extension if we decide not to do llama.cpp yet). All three require explicit operator approval at entry.
...
**Phase 31CF — Qwen2.5-1.5B Real-Activation Capture via llama.cpp Instrumentation (Option B), only if explicitly requested and operator permission is granted for llama.cpp modification and rebuild on this host.** Phase 31CE Option A passed with `PASS_31CE_REAL_ACTIVATION_OPTION_A_MULTIPROMPT_MULTILAYER_CLEAN`. 31CE extended 31CD from a 1-pair (L0, last-prefill-token, "The capital of France is") micro-probe to a 9-pair (3 prompt × 3 layer × last-prefill-token) micro-probe on the same HF-derived real-activation proxy with the same corrected Q2_K + SDIR replay pipeline and the same Option A HF-derived capture mechanism; 9/9 pairs memory-positive, 9/9 cos non-negative, 9/9 MAE non-worsening, 0/9 severe. **Option A is NOT** the path to "exact Q4_K_M GGUF runtime activation capture" — that is **Option B** (Phase 31CF), which is the only path that can claim exact Q4_K_M GGUF runtime activation behavior and requires explicit operator permission to modify and rebuild `~/llama.cpp` on this host. Alternative next phases: **31CE-R (Real-Activation Option A Repair / Prompt-Layer Diagnosis), only if explicitly requested** (recovery branch if 31CE is rejected post-merge or needs per-pair diagnosis); **31CG (Option A Larger Prompt/Token Sensitivity Planning), only if explicitly requested** (planning-only extension to a larger prompt set or token-position sweep, if we decide not to do llama.cpp yet). All three require explicit operator approval at entry; the agent does NOT proceed to any without a new request.
