...
- **Current allowed scientific next phase:** **Phase 31CC — Qwen2.5-1.5B Real-Activation Capture Planning, only if explicitly requested.** 31CB's recommended default (per 31CB reasoning: after stale provenance cleanup, the next scientific gap is real prompt-derived activation behavior, not more random-activation tensor harness work). Alternative 31CC option (operator may choose instead): 31CC Runtime Artifact Format / Loader Planning. 31CB did NOT run validation, generation, inference, or llama.cpp runtime integration. 31CB is a provenance + working-tree hygiene phase.
...
**Phase 31CC — Qwen2.5-1.5B Real-Activation Capture Planning, only if explicitly requested** (recommended default per 31CB's reasoning).

Rationale:
- Phase 31CB PASSED with classification `PASS_31CB_STALE_FILE_PROVENANCE_CLEANED`.
- 31CB is a **provenance / working-tree hygiene phase only**. NO new validation, NO generation, NO inference, NO llama.cpp runtime integration, NO modification of scientific results, NO tags.
- 31CB inspected 5 stale 31BH-R2 / 31BJ files and moved all 5 to `rescue/stale_phase31bh_31bj/` with a provenance README. **No file was committed. No file was deleted. No file was promoted to canonical evidence. No scientific/numeric claim was changed.** The 0.5B and 1.5B frozen evidence tiers remain exactly as 31CA froze them.
- Per-file disposition table: 5 of 5 → MOVE_TO_RESCUE. 0 → COMMIT_AS_HISTORICAL_RECORD. 0 → DELETE_AFTER_CONFIRMATION. 0 → LEAVE_UNTRACKED_WITH_REASON.
- The 31CB cleanup was driven by the specific contradictions found during inspection:
  - the 31BH-R2 MD header claims `PASS_31BH_R2_HISTORICAL_ANCHORS_REPRODUCED`, but the corresponding 31BH-R2 JSON says `BLOCKED_31BH_R2_ANCHORS_STILL_MISMATCH` with `historical_reproduction: {L21_S9: false, L21_S0: false}`
  - the 31BJ `ANCHOR_ONLY` JSON contains a single `corrected_ceil_per_row` row with `margin: -261638` (memory-NEGATIVE for that single Q2_K W_low variant) which could be misread as contradicting the 0.5B/1.5B memory-positive aggregate without context
  - the 31BH-R2 SOT entry uses a different classification name (`PASS_31BH_R2_FIX_RUNTIME_DISPATCH_REPAIRED`) referring to a "runtime dispatch fix" rather than anchor reproduction — the MD-vs-JSON classification drift is real but the SOT entry is the committed audit-trail fact and cannot be re-edited
- The rescue README (`rescue/stale_phase31bh_31bj/README.md`, 12.9 KB) is the canonical warning to future agents: **do not cite the rescue files as current evidence; do not import the rescue runner code; use the current `src/corrected_q2k_policy.py` constants and the current `src/q2k_backend.py` + `src/phase31x_manifest_runtime.py` implementations.**
- 31CB scope (recommended default for next): **Phase 31CC — Real-Activation Capture Planning** — after stale provenance cleanup, the next scientific gap is real prompt-derived activation behavior, not more random-activation tensor harness work.
- Alternative 31CC option (operator may choose instead): 31CC Runtime Artifact Format / Loader Planning (prerequisite for any runtime-integration claim).
- Prior accepted numeric results (0.5B 31AY / 31BA / 31BM / 31BN / 31BO; 1.5B 31BS / 31BT / 31BU / 31BV / 31BW / 31BX / 31BY / 31BZ; 31CA freeze) are unchanged. The 31CB cleanup is documentation and working-tree hygiene; it does not modify any prior accepted fact.
- Stop conditions: orientation regression (re-verify 31BT result), regression failure, model file tracking, scope creep, any aggregate / generation / runtime / quality / behavior / production / speedup / latency / larger-model claim triggered without explicit approval, any 0.5B-vs-1.5B / 1.5B-vs-3B / 1.5B-vs-7B / cross-family claim triggered without explicit approval, any larger-model validation triggered without explicit approval, any re-validation of historical 31AY/31BA anchors without an explicit future phase, or any 1.5B result generalized beyond the 31BU/31BV/31BW/31BX/31BY/31BZ scope.
- The 31CB artifacts (`docs/PHASE31CB_STALE_FILE_PROVENANCE_CLEANUP.md`, `rescue/stale_phase31bh_31bj/README.md`, the 5 moved files under `rescue/stale_phase31bh_31bj/`, and the SOT edit) are untracked. They will be committed only if Matt explicitly approves.
